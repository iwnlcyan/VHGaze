...
namespace RealisticEyeMovements {
	...
	public class EyeAndHeadAnimator : MonoBehaviour
	{
		...
		{
			return Time.time > timeOfEyeMovementStart + 
				Mathf.Max(startLeftEyeHorizDuration, Mathf.Max(startLeftEyeVertDuration, Mathf.Max(startRightEyeHorizDuration, startRightEyeVertDuration))) +
				kAttentionChangeDeadTimeAfterSaccadeEnd;
		}
		...
		void UpdateEyeMovement(float deltaTime)
		{
			...
			//*** Prevent cross-eyes
			{
				if ( eyeDistanceScale > 0 )
				{
					...
					float distance = eyeCenterToTarget.magnitude;
					float corrDistMax = eyeDistanceScale * (isLookingAtFace ? 2f : 0.6f);
					float corrDistMin = eyeDistanceScale * (isLookingAtFace ? 1.5f : 0.2f);
