...
namespace RealisticEyeMovements {
	...
	public class LookTargetController : MonoBehaviour
	{
		...
		// Keep looking at the player until new command
		...
		{
			LookAtPlayer(duration, EyeAndHeadAnimator.kDefaultHeadLatency);
		}
		
		
		// To keep looking at the player until new command, set duration to -1
		public void LookAtPlayer(float duration, float headLatency)
		{
		...
		{
			LookAroundIdly(EyeAndHeadAnimator.kDefaultHeadLatency);
		}
		
		
		public void LookAroundIdly(float headLatency)
		{
			if (false == isInitialized)
				Initialize();
		...
		// keep looking at the poi until new command
		public void LookAtPoiDirectly( Transform poiXform )
		{
			LookAtPoiDirectly(poiXform, -1, EyeAndHeadAnimator.kDefaultHeadLatency);
		}
		
		
		// To keep looking at the poi until new command, set duration to -1
		public void LookAtPoiDirectly( Transform poiXform, float duration )
		{
			LookAtPoiDirectly(poiXform, duration, EyeAndHeadAnimator.kDefaultHeadLatency);
		}
		
		
		public void LookAtPoiDirectly( Transform poiXform, float duration, float headLatency )
		{
			if (false == isInitialized)
				Initialize();
		...
		{
			LookAtPoiDirectly(poi, -1, EyeAndHeadAnimator.kDefaultHeadLatency);
		}
		
		
		// To keep looking at the poi until new command, set duration to -1
		public void LookAtPoiDirectly( Vector3 poi, float duration )
		{
			LookAtPoiDirectly(poi, duration, EyeAndHeadAnimator.kDefaultHeadLatency);
		}
		
		
		// To keep looking at the poi until new command, set duration to -1
		public void LookAtPoiDirectly( Vector3 poi, float duration, float headLatency )
		{
			if (false == isInitialized)
				Initialize();
		...
		void UpdateLookTarget(bool shouldLookAwayFromPlayer=false)
		{
			...
			{
				float duration = Random.Range(Mathf.Min(minLookTime, maxLookTime), Mathf.Max(minLookTime, maxLookTime));
				if ( state != State.LookingAtPlayer )
					LookAtPlayer(duration);
				else
					nextChangePOITime = Time.time + duration;
			}
			else
				LookAroundIdly();
		}
		...
		void UpdateNativeVREyePositions()
		{
			...
			if ( false == useNativeVRSupport || false == Utils.IsVRDeviceRunning() || usedThirdPersonPlayerEyeCenter != null)
			{
